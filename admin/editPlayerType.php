<?php 
	...
	if($sport_data->success == 1){
 	...
 	$type_data = array();
 	$type_data = $action->getPlayerTypebyId($_GET['id']);
 	...
 	if ($_SERVER['REQUEST_METHOD'] == 'POST'){
		...
		$t_data = $action->updatePlayerType($_POST);
				...
				<div class="m-portlet m-portlet--tab">
					...
					<input type="hidden" name="id" value="<?php echo $_GET['id']; ?>">
						<div class="m-portlet__body">
							...
							<div class="form-group m-form__group row">
								<label for="example-text-input" class="col-2 col-form-label">Player Type</label>
								<div class="col-4">
									<input class="form-control m-input m-input--solid" type="text" id="player_type" name="player_type" value="<?php echo $type->player_type; ?>">
								</div>
							</div>
						</div>
