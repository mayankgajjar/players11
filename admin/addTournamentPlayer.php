...
<?php 
	...
	if($sport_data->success == 1){
 	...
 	if ($_SERVER['REQUEST_METHOD'] == 'POST'){
		...
		$p_data = $action->addPlayer($_POST, $files);
					...
					<form class="m-form m-form--fit m-form--label-align-right" method="POST" action="" enctype="multipart/form-data">
						...
						<div class="m-portlet__body">
							<div class="form-group m-form__group row">
								...
								<div class="col-4">
									<input class="form-control m-input m-input--solid" type="text" id="name" name="name" required>
								</div>
								<label for="example-text-input" class="col-2 col-form-label">Player Short Name</label>
								<div class="col-4">
									<input class="form-control m-input m-input--solid" type="text" id="short_name" name="short_name" required>
								</div>
							</div>
						</div>
						...
						<div class="m-portlet__body">
							<div class="form-group m-form__group row">
								<label for="example-text-input m-input--solid" class="col-2 col-form-label">API Code</label>
								<div class="col-4">
									<input class="form-control m-input m-input--solid" type="text" id="api_code" name="api_code" required>
								</div>
								<label for="example-text-input" class="col-2 col-form-label">Credits</label>
								<div class="col-4">
									<input class="form-control m-input m-input--solid" type="text" id="credits" name="credits" >
								</div>
							</div>
						</div>
								...
								<div class="col-4">
									<select class="custom-select form-control m-input--solid" name="team_key" id="team_key">
										...
										<?php foreach ($team as $key => $value) { ?>
											<option value="<?php echo $value->team_key; ?>"><?php echo $value->name; ?></option>
										<?php } ?>
									</select>
								</div>
								...
								<div class="col-4">
									<select class="custom-select form-control m-input--solid" name="role" id="role">
										...
										<?php foreach ($role as $key => $value) { ?>
											<option value="<?php echo $value->player_type; ?>" required><?php echo $value->player_type; ?></option>
										<?php } ?>
									</select>
								</div>
							</div>
						...
						<div class="m-portlet__body">
							<div class="form-group m-form__group row">
								<label for="example-text-input m-input--solid" class="col-2 col-form-label">Profile Picture</label>
								<div class="col-4">
