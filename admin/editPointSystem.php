<?php 
	...
	if($player_type_data->success == 1){
 	...
 	$point_data = array();
 	$point_data = $action->getPointSystemById($_GET['id']);
 	$point_data = json_decode($point_data);
 	if($point_data->success == 1){
		$point = $point_data->data;
		$ptype_array = json_decode($point->role);
	}

 	if ($_SERVER['REQUEST_METHOD'] == 'POST'){
		$t_data = $action->editPointSystem($_POST);
					...
					<form class="m-form m-form--fit m-form--label-align-right" method="POST" action="">
						...
						<div class="m-portlet__body">
								...
								<div class="col-4">
									<select class="form-control m-input m-input--square m-input--solid" id="point_type_id" name="point_type_id" required="required">
										...
										<?php foreach ($point_type as $key => $value) { ?>
											<option value="<?php echo $value->id; ?>" <?php if($value->id == $point->point_type_id) { echo 'selected'; }?>><?php echo $value->point_type; ?></option>
										<?php } ?>
									</select>
								</div>
							</div>
							<div class="form-group m-form__group row">
								...
								<div class="col-4">
									<input class="form-control m-input m-input--solid" type="text" id="title" name="title" required="required" value="<?php echo $point->title; ?>">
								</div>
								...
								<div class="col-4">
									<select class="form-control m-input m-input--square m-input--solid" id="point_unit" name="point_unit" required="required">
										...
										<option value="0" <?php if($point->point_unit == 0) { echo 'selected'; }?>> Plus </option>
										<option value="1" <?php if($point->point_unit == 1) { echo 'selected'; }?>> Minus </option>
									</select>
								</div>
							</div>
							<div class="form-group m-form__group row">
								...
								<div class="col-4">
									<select class="form-control m-input m-input--square m-input--solid" id="role" name="role[]" multiple="multiple">
										<?php foreach ($player_type as $key => $value) { ?>
											<option value="<?php echo $value->id; ?>" <?php echo (in_array($value->id, $ptype_array) ? 'Selected' : '') ?> ><?php echo $value->player_type; ?></option>
										<?php } ?>
									</select>
								</div>
								<label for="example-text-input" class="col-2 col-form-label">Point</label>
								<div class="col-4">
									<input class="form-control m-input m-input--solid" type="text" id="total_point" name="total_point" required="required" value="<?php echo $point->total_point; ?>">
								</div>
							</div>
							...
							<div class="form-group m-form__group row">
								...
								<div class="col-4">
									<input class="form-control m-input m-input--solid" type="text" id="code" name="code" value="<?php echo $point->code; ?>">
								</div>
