...
// Connect to database
	...
	{
		case 'tournamentList':
			// Retrive Tournament List
			if(!empty($_REQUEST["id"]))
			{
				$id=intval($_REQUEST["id"]);
				fb_get_tournament($id);
			} else {
				fb_get_tournament();
			}
			break;
	...
	function create_team(){
		...
		$user_team_player_insert = "INSERT INTO user_team_player(user_tean_id, user_id, match_id, match_key, team_no, player_key_json,player_id_json) VALUES (".$user_tean_id.",".$user_id.",".$match_id[0].",".$match_key[0].",
			".$team_no.",".$pjson.",".$player_id_json.")";
